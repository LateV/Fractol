...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/16 20:48:29 by vibondar          #+#    #+#             */
/*   Updated: 2018/03/16 20:48:31 by vibondar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int			ft_tree(t_fract *fract, double re, double im)
{
	int		n;
	double	xy;
	double	twoxy;
	...
	while (i < IHI / THREADS * (thread->thread_id + 1))
	{
		...
		while (++j < IWI)
		{
			...
			ft_putpixel(thread->fract, j, i,
				ft_col(ft_tree(thread->fract, re, im), thread->fract));
		}
		i++;
	}
	return (NULL);
